...
// Can be used for either SPI0 or SPI1
void SPIClass::begin(uint32_t spi_periph,uint32_t cs,uint32_t miso,uint32_t mosi,uint32_t clk) {

  	rcu_periph_clock_enable((spi_periph==SPI0)? RCU_SPI0 : RCU_SPI1);

    pinCS = cs;

  	pinMode(cs, OUTPUT);
   	if (miso > 0) pinMode(miso, OUTPUT_AF_PP);
...
uint16_t SPIClass::transfer16(uint16_t _data) {
 ...
 /* send half word through the SPI0 peripheral */
  ...
  /* wait to receive a half word */
    ...
    return d ;
  #else
  res = transfer((byte) (d >> 8));
  res = (res << 8) | transfer((byte) (d & 0xff));
  return res;
  #endif
